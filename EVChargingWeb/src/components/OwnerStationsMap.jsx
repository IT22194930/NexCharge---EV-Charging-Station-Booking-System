...
export default function OwnerStationsMap() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setStations(
          res.data.filter((s) => s.isActive && s.latitude && s.longitude)
        );
      } catch {
  ...
  const center = userPosition || {
    lat: stations[0]?.latitude || 6.927079,
    lng: stations[0]?.longitude || 79.861244,
  }; // default Colombo fallback
      ...
      <div className="flex items-center mb-6">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Find and book one of the closest active stations
          </p>
        </div>
      </div>
        ...
        <div className="lg:col-span-2">
          <div className="bg-white rounded-2xl shadow relative overflow-hidden">
            ...
            <div className="p-4 relative z-10">
              {!isLoaded ? (
                <div className="h-[450px] flex items-center justify-center text-gray-500 animate-pulse">
                  Loading map...
                </div>
              ) : (
                <GoogleMap
                    ...
                    >
                      <div className="max-w-[230px]">
                        ...
                        <p className="text-[11px] text-gray-500 mb-2">
                          Type: {selectedStation.type} • Slots:{" "}
          ...
          <div className="bg-white rounded-2xl shadow p-5 h-full flex flex-col">
            ...
            >
              {nearestStations.map((s, idx) => (
                <div
                  key={s.id}
                  className="group border border-gray-100 hover:border-emerald-300 rounded-xl p-3 transition-all bg-white hover:shadow-sm"
                >
                  <div className="flex items-start justify-between">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center mb-1">
                        <span className="inline-flex items-center justify-center w-5 h-5 text-[10px] font-semibold rounded-full bg-emerald-100 text-emerald-700 mr-2">
                          {idx + 1}
                        </span>
              ...
              {nearestStations.length === 0 && (
                <p className="text-sm text-gray-500 py-4 text-center">
                  No active stations with coordinates.
                </p>
              )}
            </div>
          </div>
